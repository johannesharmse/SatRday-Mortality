...
if(nrow(training.df) > 2500){
  ...
  if (nrow(training.df2) < 2500){
    training.df = rbind(training.df2, training.df3[sample(nrow(training.df3), 10000 - nrow(training.df2)), ])
  }else{
...
saveRDS(finalModel, paste('C:/Users/jharmse/Documents/R/SatRday/Predictive - ML/', 'finalModel', '.RDS', sep = ''))

save(list = ls(all.names = TRUE), file = "C:/Users/jharmse/Documents/R/SatRday/Predictive - ML/.RData", envir = .GlobalEnv)
